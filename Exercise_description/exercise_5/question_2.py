...
with open(filename) as f:
    bgp_neigbours = f.read()

pattern= (r"^BGP neighbor is (?P<bgp_neighbor>[\d\.]+),\s+remote AS (?P<remote_as>\d+),.*$")
match = re.search(pattern, bgp_neighbours, flags=re.M)
if match:
    bgp_neighbor_ip = match.group("bgp_neighbor")
    remote_as = match.group("remote_as")

pattern= (r"^\s+BGP version (?P<bgp_version>\d+), remote router ID (?P<remote_rid>\S+).*$")
match= re.search(pattern, bgp_neighbours, flags= re.M)
if match:
